...
function rotatePoint(
  ...
  const x =
    center.x +
    (pt.x - center.x) * Math.cos(theta) +
    (pt.y - center.y) * Math.sin(theta)
  const y =
    center.y -
    (pt.x - center.x) * Math.sin(theta) +
    (pt.y - center.y) * Math.cos(theta)
  return { x, y }
}
...
export function rotatedRectBounds(rect: Rect, rotationDegrees: number): Rect {
  ...
  const bottomRight = addPoints(rect.origin, {
    x: rect.size.w,
    y: rect.size.h,
  })
