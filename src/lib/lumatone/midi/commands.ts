...
import {
  ...
  createSysexToggle,
  createTableSysex,
  createExtendedKeyColourSysex,
  createExtendedMacroColourSysex,
} from './sysex'
 ...
 */
export const setKeyFunctionParameters = (
  boardIndex: BoardIndex,
  keyIndex: number,
  noteOrCCNum: number,
  midiChannel: number,
  keyType: number,
  faderUpIsNull: boolean
): EncodedSysex => {
  midiChannel = (midiChannel - 1) & 0xf
  const typeByte = faderUpIsNull ? (1 << 4) | keyType : keyType
  return createSysex(
    boardIndex,
    C.CHANGE_KEY_NOTE,
    keyIndex,
 ...
 */
export const setKeyLightParameters = (
  boardIndex: BoardIndex,
  keyIndex: number,
  red: number,
  green: number,
...
export const saveProgram = (presetNumber: number): EncodedSysex => {
  if (presetNumber < MIN_PRESET || presetNumber > MAX_PRESET) {
    throw new Error(
      `Invalid preset number. Range: [${MIN_PRESET}, ${MAX_PRESET}]`
    )
  }
...
export const setExpressionPedalSensivity = (value: number): EncodedSysex =>
  createSysex(
    BoardIndex.SERVER,
    C.SET_FOOT_CONTROLLER_SENSITIVITY,
    value & 0x7f
  )
 ...
 */
export const setMacroButtonActiveColour = (
  red: number,
  green: number,
  blue: number
): EncodedSysex =>
 ...
 */
export const setMacroButtonInactiveColour = (
  red: number,
  green: number,
  blue: number
): EncodedSysex =>
 ...
 */
export const saveVelocityConfig: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.SAVE_VELOCITY_CONFIG
)

/**
 * CMD 0Ah: Reset velocity config to value from EEPROM
 */
export const resetVelocityConfig: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.RESET_VELOCITY_CONFIG
)
 ...
 * @deprecated
 */
export const saveFaderConfiguration: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.SAVE_FADER_CONFIG
)
 ...
 */
export const resetFaderConfig: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.RESET_FADER_CONFIG
)
 ...
 */
export const calibrateAfterTouch: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.CALIBRATE_AFTERTOUCH
)
 ...
 * @deprecated
 */
export const saveAftertouchConfig: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.SAVE_AFTERTOUCH_CONFIG
)

/**
 * CMD 12h: Reset the aftertouch lookup table back to its factory aftertouch settings.
 */
export const resetAftertouchConfig: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.RESET_AFTERTOUCH_CONFIG
)
 ...
 */
export const maxFaderThresholdRequest = (
  boardIndex: BoardIndex
): EncodedSysex => createSysex(boardIndex, C.GET_MAX_THRESHOLD)
 ...
 */
export const maxAftertouchThresholdRequest = (
  boardIndex: BoardIndex
): EncodedSysex => createSysex(boardIndex, C.GET_AFTERTOUCH_MAX)

/**
 * CMD 1Ch: Get back flag whether or not each key of target board meets minimum threshold
 */
export const keyValidityParametersRequest = (
  boardIndex: BoardIndex
): EncodedSysex => createSysex(boardIndex, C.GET_KEY_VALIDITY)

/**
 * CMD 1Dh: Read back the current velocity look up table of the keyboard.
 */
export const velocityConfigRequest: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.GET_VELOCITY_CONFIG
)

/**
 * CMD 1Eh: Read back the current fader look up table of the keyboard.
 */
const faderConfigRequest: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.GET_FADER_CONFIG
)
 ...
 */
export const aftertouchConfigRequest: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.GET_AFTERTOUCH_CONFIG
)

/**
 * CMD 20h: Set the velocity interval table, 127 12-bit values
 */
export const setVelocityIntervalConfig = (
  velocityIntervalTable: number[]
): EncodedSysex => {
 ...
 */
export const velocityIntervalConfigRequest: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.GET_VELOCITY_INTERVALS
)
 ...
 */
export const getSerialIdentityRequest: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.GET_SERIAL_IDENTITY,
  TEST_ECHO
)
 ...
 */
export const calibrateKeys: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.CALIBRATE_KEYS
)
 ...
 */
export const sendCalibratePitchModWheel = (
  startCalibration: boolean
): EncodedSysex =>
  createSysexToggle(
    BoardIndex.SERVER,
    C.CALIBRATE_PITCH_MOD_WHEEL,
    startCalibration
  )
 ...
 */
export const setKeyMaximumThreshold = (
  boardIndex: BoardIndex,
  maxThreshold: number,
  aftertouchMax: number
): EncodedSysex => {
  if (maxThreshold > 0xfe) maxThreshold &= 0xfe
  if (aftertouchMax > 0xfe) aftertouchMax &= 0xfe
  return createSysex(
    boardIndex,
    C.SET_KEY_MAX_THRESHOLD,
    maxThreshold >> 4,
    maxThreshold & 0xf,
    aftertouchMax >> 4,
    aftertouchMax & 0xf
 ...
 */
export const setKeyMinimumThreshold = (
  boardIndex: BoardIndex,
  minThresholdHigh: number,
  minThresholdLow: number
): EncodedSysex => {
  if (minThresholdHigh > 0xfe) minThresholdHigh &= 0xfe
  if (minThresholdLow > 0xfe) minThresholdLow &= 0xfe
  return createSysex(
    boardIndex,
    C.SET_KEY_MIN_THRESHOLD,
    minThresholdHigh >> 4,
    minThresholdHigh & 0xf,
    minThresholdLow >> 4,
    minThresholdLow & 0xf
  )
}
 ...
 */
export const setFaderKeySensitivity = (
  boardIndex: BoardIndex,
  sensitivity: number
): EncodedSysex => {
 ...
 */
export const setAftertouchKeySensitivity = (
  boardIndex: BoardIndex,
  sensitivity: number
): EncodedSysex => {
  if (sensitivity > 0xfe) sensitivity &= 0xfe
  return createSysex(
    boardIndex,
    C.SET_KEY_AFTERTOUCH_SENSITIVITY,
    sensitivity >> 4,
    sensitivity & 0xf
  )
}
 ...
 * @deprecated
 */
export const saveLumatoneConfig: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.SAVE_LUMATOUCH_CONFIG
)

/**
 * CMD 2Fh: Reset the Lumatouch table back to factory settings
 */
export const resetLumatouchConfig: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.RESET_LUMATOUCH_CONFIG
)

/**
 * CMD 30h: Read back the Lumatouch table
 */
export const lumatouchConfigRequest: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.GET_LUMATOUCH_CONFIG
)

/**
 * CMD 31h: This command is used to read back the current Lumatone firmware revision.
 */
export const getFirmwareRevisionRequest: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.GET_FIRMWARE_REVISION
)

/**
 * CMD 32h: Set the thresold from key’s min value to trigger CA - 004 submodule CC events, ranging from 0x00 to 0xFE
 */
export const setCCActiveThreshold = (
  boardIndex: BoardIndex,
  sensitivity: number
): EncodedSysex => {
  if (sensitivity > 0xfe) sensitivity &= 0xfe
 ...
 */
export const setKeySampling = (
  boardIndex: BoardIndex,
  turnSamplingOn: boolean
): EncodedSysex =>
 ...
 */
export const resetWheelsThresholds: EncodedSysex = createSysex(
  BoardIndex.SERVER,
  C.RESET_WHEELS_THRESHOLD
)
...
export const setPitchWheelZeroThreshold = (threshold: number): EncodedSysex => {
  ...
  return createSysex(
    BoardIndex.SERVER,
    C.SET_PITCH_WHEEL_CENTER_THRESHOLD,
    threshold
  )
}
 ...
 */
export const resetExpressionPedalBounds = createSysex(
  BoardIndex.SERVER,
  C.RESET_EXPRESSION_PEDAL_BOUNDS
)
 ...
 */
export const getBoardSensitivityValues = (
  boardIndex: BoardIndex
): EncodedSysex => createSysex(boardIndex, C.GET_BOARD_SENSITIVITY_VALUES)

/**
 * CMD 3Ch: Set the MIDI channels for peripheral controllers
 */
export const setPeripheralChannels = (
  pitchWheelChannel: number,
  modWheelChannel: number,
  expressionChannel: number,
  sustainChannel: number
): EncodedSysex => {
 ...
 */
export const getPeripheralChannels = createSysex(
  BoardIndex.SERVER,
  C.GET_PERIPHERAL_CHANNELS
)
 ...
 */
export const setAfterTouchTriggerDelay = (
  boardIndex: BoardIndex,
  aftertouchTriggerValue: number
): EncodedSysex =>
  createSysex(
    boardIndex,
    C.SET_AFTERTOUCH_TRIGGER_DELAY,
 ...
 */
export const getAftertouchTriggerDelayRequest = (
  boardIndex: BoardIndex
): EncodedSysex => createSysex(boardIndex, C.GET_AFTERTOUCH_TRIGGER_DELAY)
 ...
 */
export const setLumatouchNoteOffDelay = (
  boardIndex: BoardIndex,
  delayValue: number
) =>
  createSysex(
    boardIndex,
    C.SET_LUMATOUCH_NOTE_OFF_DELAY,
    (delayValue >> 8) & 0xf,
    (delayValue >> 4) & 0xf,
    delayValue & 0xf
  )
 ...
 */
export const getLumatouchNoteOffDelayRequest = (
  boardIndex: BoardIndex
): EncodedSysex => createSysex(boardIndex, C.GET_LUMATOUCH_NOTE_OFF_DELAY)

/**
 * CMD 44h: Get the current expression pedal ADC threshold value
 */
export const getExpressionPedalADCThresholdRequest = createSysex(
  BoardIndex.SERVER,
  C.GET_EXPRESSION_PEDAL_THRESHOLD
)
...
export const setExpressionPedalADCThreshold = (thresholdValue: number) =>
  createSysex(
    BoardIndex.SERVER,
    C.SET_EXPRESSION_PEDAL_THRESHOLD,
