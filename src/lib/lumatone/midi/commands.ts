 ...
 */
 export const setKeyFunctionParameters = (boardIndex: number, keyIndex: number, noteOrCCNum: number, midiChannel: number, keyType: number, faderUpIsNull: boolean): EncodedSysex => {
 ...
 */
export const setKeyLightParameters = (boardIndex: number, keyIndex: number, red: number, green: number, blue: number): EncodedSysex => 
...
export const saveProgram = (presetNumber: number): EncodedSysex => {
  ...
  return createSysEx(C.BOARD_SERVER, C.SAVE_PROGRAM, presetNumber)
}
...
export const setExpressionPedalSensivity = (value: number): EncodedSysex => 
  createSysEx(C.BOARD_SERVER, C.SET_FOOT_CONTROLLER_SENSITIVITY, value & 0x7f)
...
export const invertFootController = (value: boolean): EncodedSysex => 
  createSysExToggle(C.BOARD_SERVER, C.INVERT_FOOT_CONTROLLER, value)
...
export const setLightOnKeyStrokes = (active: boolean): EncodedSysex =>
  createSysExToggle(C.BOARD_SERVER, C.SET_LIGHT_ON_KEYSTROKES, active)
...
export const saveVelocityConfig: EncodedSysex =
  createSysEx(C.BOARD_SERVER, C.SAVE_VELOCITY_CONFIG)

/**
 * CMD 0Ah: Reset velocity config to value from EEPROM
 */
export const resetVelocityConfig: EncodedSysex =
  createSysEx(C.BOARD_SERVER, C.RESET_VELOCITY_CONFIG)
...
export const saveAftertouchConfig: EncodedSysex =
  createSysEx(C.BOARD_SERVER, C.SAVE_AFTERTOUCH_CONFIG)
...
export const resetAftertouchConfig: EncodedSysex =
  createSysEx(C.BOARD_SERVER, C.RESET_AFTERTOUCH_CONFIG)

/**
 * CMD 13h: Read back the current red intensity of all the keys of the target board.
 */
export const redLEDConfigRequest = (boardIndex: number): EncodedSysex =>
 ...
 */
export const greenLEDConfigRequest = (boardIndex: number): EncodedSysex =>
 ...
 */
export const blueLEDConfigRequest = (boardIndex: number): EncodedSysex =>
 ...
 */
export const channelConfigRequest = (boardIndex: number): EncodedSysex =>
 ...
 */
export const noteConfigRequest = (boardIndex: number): EncodedSysex =>
 ...
 */
export const maxAftertouchThresholdRequest= (boardIndex: number): EncodedSysex =>
 ...
 */
export const keyValidityParametersRequest = (boardIndex: number): EncodedSysex =>
...
export const velocityIntervalConfigRequest: EncodedSysex =
  createSysEx(C.BOARD_SERVER, C.GET_VELOCITY_INTERVALS)

/**
 * CMD 22h: Read back the fader type of all keys on the targeted board.
 */
export const faderTypeConfigRequest = (boardIndex: number): EncodedSysex =>
...
export const getSerialIdentityRequest: EncodedSysex =
  createSysEx(C.BOARD_SERVER, C.GET_SERIAL_IDENTITY, C.TEST_ECHO)
...
export const sendCalibratePitchModWheel = (startCalibration: boolean): EncodedSysex =>
  createSysExToggle(C.BOARD_SERVER, C.CALIBRATE_PITCH_MOD_WHEEL, startCalibration)
...
export const setModWheelSensitivity = (sensitivity: number): EncodedSysex => {
  ...
  createSysEx(C.BOARD_SERVER, C.SET_MOD_WHEEL_SENSITIVITY, sensitivity)
}
...
export const setPitchBendSensitivity = (sensitivity: number): EncodedSysex => {
  ...
  return createSysEx(C.BOARD_SERVER, C.SET_PITCH_WHEEL_SENSITIVITY, sensitivity >> 7, sensitivity & 0x7f)
}

/**
 * CMD 29h: Set abs. distance from max value to trigger CA-004 submodule key events, ranging from 0x00 to 0xFE
 */
export const setKeyMaximumThreshold = (boardIndex: number, maxThreshold: number, aftertouchMax: number): EncodedSysex => {
 ...
 */
export const setKeyMinimumThreshold = (boardIndex: number, minThresholdHigh: number, minThresholdLow: number): EncodedSysex => {
 ...
 */
export const setFaderKeySensitivity = (boardIndex: number, sensitivity: number): EncodedSysex => {
 ...
 */
export const setAftertouchKeySensitivity = (boardIndex: number, sensitivity: number): EncodedSysex => {
  if (sensitivity > 0xfe) sensitivity &= 0xfe
...
export const getFirmwareRevisionRequest: EncodedSysex =
  createSysEx(C.BOARD_SERVER, C.GET_FIRMWARE_REVISION)

/**
 * CMD 32h: Set the thresold from key’s min value to trigger CA - 004 submodule CC events, ranging from 0x00 to 0xFE
 */
export const setCCActiveThreshold = (boardIndex: number, sensitivity: number): EncodedSysex => {
  if (sensitivity > 0xfe) sensitivity &= 0xfe
...
export const ping = (value: number): EncodedSysex => {
  ...
  return createSysEx(C.BOARD_SERVER, C.LUMA_PING, C.TEST_ECHO,
    (value >> 14) & 0x7f,
    (value >> 7) & 0x7f, 
    value & 0x7f
  )
 ...
 */
export const resetBoardThresholds = (boardIndex: number): EncodedSysex =>
  createSysEx(boardIndex, C.RESET_BOARD_THRESHOLDS)

/**
 * CMD 35h: Enable/disable key sampling over SSH for the target key and board
 */
export const setKeySampling = (boardIndex: number, turnSamplingOn: boolean): EncodedSysex => 
...
export const resetWheelsThresholds: EncodedSysex =
  createSysEx(C.BOARD_SERVER, C.RESET_WHEELS_THRESHOLD)
...
export const setPitchWheelZeroThreshold = (threshold: number): EncodedSysex => {
  ...
  return createSysEx(C.BOARD_SERVER, C.SET_PITCH_WHEEL_CENTER_THRESHOLD, threshold)
}
...
export const setPeripheralChannels = (pitchWheelChannel: number, modWheelChannel: number, expressionChannel: number, sustainChannel: number): EncodedSysex => {
  ...
  return createSysEx(C.BOARD_SERVER, C.SET_PERIPHERAL_CHANNELS,
...
export const getPeripheralChannels =
  createSysEx(C.BOARD_SERVER, C.GET_PERIPHERAL_CHANNELS)
 ...
 */
export const setAfterTouchTriggerDelay = (boardIndex: number, aftertouchTriggerValue: number): EncodedSysex =>
 ...
 */
export const getAftertouchTriggerDelayRequest = (boardIndex: number): EncodedSysex =>
 ...
 */
export const setLumatouchNoteOffDelay = (boardIndex: number, delayValue: number) =>
 ...
 */
export const getLumatouchNoteOffDelayRequest = (boardIndex: number): EncodedSysex =>
...
export const getExpressionPedalADCThresholdRequest = 
  createSysEx(C.BOARD_SERVER, C.GET_EXPRESSION_PEDAL_THRESHOLD)
...
export const setExpressionPedalADCThreshold = (thresholdValue: number) =>
  createSysEx(C.BOARD_SERVER, C.SET_EXPRESSION_PEDAL_THRESHOLD, 
...
export const invertSustainPedal = (invert: boolean): EncodedSysex => 
  createSysExToggle(C.BOARD_SERVER, C.INVERT_SUSTAIN_PEDAL, invert)
